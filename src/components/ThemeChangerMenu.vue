    ...
    <div class="text-center">
      <v-menu v-model="menu" :close-on-content-click="false" location="end">
        ...
        <!-- <v-card min-width="300">
          <v-list>
            <v-list-item prepend-avatar="./src/assets/Beard_DaBowe.svg" title="Da Bowe" subtitle="Mr Bowe if you nasty">
        ...
        <v-card-text>
          ...
          <v-card class="my-2" @click="setTheme(myTheme)" v-for="(myTheme, index) in myThemes" :key="index">
						...
						<v-list-item>
              <v-list-item-content>
                <v-list-item-title class="font-weight-bold"> {{ myTheme.name }}</v-list-item-title>
              </v-list-item-content>
              <v-list-item-action>
                <v-avatar color="success" size="30" v-if="$vuetify.theme.themes.name === myTheme.name">
                  <v-icon>mdi-check</v-icon>
                </v-avatar>
              </v-list-item-action>
            </v-list-item>
            <div class="my-2">
              <v-chip class="mx-1" label :color="myTheme.dark[key]" v-for="(key, index) in Object.keys(myTheme.dark)" :key="index"> {{ key }}</v-chip>
            </div>
            <div class="my-2">
              <v-chip class="mx-1" label :color="myTheme.light[key]" v-for="(key, index) in Object.keys(myTheme.light)" :key="index"> {{ key }}</v-chip>
            </div>
...
function setTheme(myTheme) {
	...
	// close menu
	// const menu = false;
	// const name =ref(theme.name);		
	// const dark = ref(theme.dark);	
	// const light = ref(theme.light);
